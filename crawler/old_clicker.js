...
(async () => {
    ...
    async function findButton(page, buttonLabel) {
        ...
        console.debug('[.] Searching for "' + buttonLabel + '" in ' + frames.length + ' frames.');
        let searchSelector = "text=" + buttonLabel;
        ...
        for (let i = 0; i < frames.length; i++) {
            ...
            try {
                const element = await frame.waitForSelector(searchSelector, { strict: false, state: 'attached', timeout: 1000 });
    ...
    async function findInputUsername(page) {
        ...
        for (let i = 0; i < frames.length; i++) {
            ...
            try {
                let inputs = await frame.locator('input[name="username"] >> visible=true');
    ...
    if(inputUsernameFound == false) {
        log.error(`[..] could not find input[name="username"], trying login buttons next`);
    } else {
        log.error(`[..] found input[name="username"], looking for oAuth providers next`);
    }
    ...
    if(inputUsernameFound == false) {
        ...
        if (loginButtonsFound.length == 0) {
            ...
            log.info(`[.] found ${loginButtonsFound.length} buttons, clicking on all until the page changes`);
    ...
    let oauthButtonLabelTemplates = ["Sign up with", "Sign in with", "Continue with", "Log in with", "Login with", "Register with"];

    let oauthProvidersRegex = "(" + oauthProviders.join('|') + ")";
    let oauthButtonLabelRegex = "(" + oauthButtonLabelTemplates.join('|') + ")";

    let oauthRegex = "/(" + oauthButtonLabelRegex + "\\s+" + oauthProvidersRegex + ")/i";
    ...
    for(let i = 0; i < tempOauthProviders.length; i++) {
        let found = resultsOauth.find(element => {
            return element.toString().toLowerCase() === tempOauthProviders[i].toLowerCase();
        });
    ...
    //
    console.log(`${outputPrefix},${expStartDt.toISOString()},${url},${loginUrl},${outputFilenameBase}-0.png,${outputFilenameBase}-1.png,${outputFilenameBase}-0.html.gz,${outputFilenameBase}-1.html.gz,${oauthProvidersBinary.join(',')}`);
